...
**Version:** 6.22 **Datei:** `index.html` (Single-File PWA) **Hosting:** GitHub Pages (statisch, kein Backend)

> **Hinweis zu dieser Revision:** Diese Aktualisierung basiert auf dem tatsächlich vorliegenden Code-Stand **v6.22** — jede hier beschriebene Änderung (v6.7–v6.22) wurde in diesem Chat direkt im Code umgesetzt und per `grep`/Funktionsansicht am Ende nochmal gegen die reale Datei verifiziert, nicht aus dem Gedächtnis rekonstruiert. Vor der nächsten Änderung trotzdem wie gehabt: aktuelle Datei hochladen und `const VER=` gegenprüfen.
...
5. Kartenanzeige (Kopfzeile, Artikel, Notiz)
...
## Kartenanzeige (Kopfzeile, Artikel, Notiz)
...
*Getestet (Playwright, automatisiert): Diakritika-Toleranz live und beim Bestätigen in beiden Modi, Buchstabendreher-Erkennung, Stufenabzug, Deep-Learn-Strafe (3→2 statt 3→0), Artikel-Platzierung auf Karten (Flagge/Level/Notiz), Karten-Höhen mit echten und leeren Daten.*
...
Am Ende dieses Chats besprochen, aber explizit **noch nicht gebaut** (Nutzer wollte erst nur Rückmeldung/Konzept):

1. **Verwechslungs-Erkennung beim Tippen:** Wenn ein falsch getipptes Wort exakt der Übersetzung einer *anderen* Vokabel entspricht (und dort nicht als Synonym hinterlegt ist), soll dieses andere Wort ebenfalls −1 Stufe bekommen, plus Erklärtext im Feedback. Geklärt: nur Tipp-Modus, nur exakter Wort-Treffer (keine Synonym-Prüfung beim anderen Wort), nur Stufenanpassung (keine Statistik-Zähler), das direkt falsch beantwortete Wort bleibt bei Totalreset (unverändert, komplett falsche Antworten waren nie −1). Letzter offener Punkt vor Umsetzung: exakte Bestätigung der Detail-Logik.
2. **Tastatur-Sprung beim Tippen (mobil):** Karten verschieben sich beim Ein-/Ausblenden der virtuellen Tastatur. Lösungsansatz besprochen: `interactive-widget=overlays-content` im Viewport-Meta-Tag (ab iOS Safari 17.4+/aktuelles Chrome unterstützt) statt bisherigem `<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">`. Zusätzlicher Diskussionspunkt des Nutzers: Karten weiter oben im Bildschirm positionieren, damit unten dauerhaft Platz für die Tastatur ist, ohne dass sich die Kartenposition je nach Tastatur-Status ändert — müsste für alle Kartentypen einheitlich sein. Noch nicht implementiert.
...
*Dokumentation zuletzt aktualisiert: August 2026, basierend auf Code-Stand **v6.22**, gegen die reale Datei verifiziert (nicht rekonstruiert).*
