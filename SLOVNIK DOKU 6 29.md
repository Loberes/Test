...
**Version:** 6.29 **Datei:** `index.html` (Single-File PWA, intern aus 3 Teilen gebaut) **Hosting:** GitHub Pages (statisch, kein Backend)
...
- PWA-fähig — `apple-mobile-web-app-capable`, installierbar. **Präzisierung seit v6.28:** Es gibt **keinen Service Worker** (wird beim Laden aktiv deregistriert/gecleart, siehe `index.html` Zeilen 511–514) und damit **keinen Offline-Betrieb** — die App braucht bei jedem Start Netz für die Sprachpaket-JSONs. `manifest.json` existiert als Datei, war aber bis v6.27 nicht per `<link rel="manifest">` in `index.html` eingebunden (seit v6.28 behoben, siehe Changelog). Für aktuelles iOS (ab iOS 26) öffnet "Zum Home-Bildschirm" ohnehin standardmäßig als eigenständige Web-App, unabhängig vom Manifest — auf Android/Chrome und älterem iOS macht das Manifest weiterhin den Unterschied für Icon/Name/Theme-Farbe bei der Installation.
...
*Dokumentation zuletzt aktualisiert: August 2026, basierend auf Code-Stand **v6.29**. v6.7–v6.27 gegen die reale Datei verifiziert (Vorgänger-Chat, inkl. Playwright); v6.28–v6.29 nur per Syntax-Check/Codesichtung (siehe Changelog-Hinweise), nicht per Playwright.*
