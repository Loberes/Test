...
**Version:** 6.30 **Dateien:** `index.html` (Single-File-App, intern aus 3 Teilen gebaut) **+ `sw.js`** (Service Worker, seit v6.30, separate Datei — siehe Changelog) **Hosting:** GitHub Pages (statisch, kein Backend)

> **Fortsetzungs-Chat (v6.28):** Neue Sitzung, aufgesetzt auf der hochgeladenen `index.html` (v6.27) und dieser Doku, wie im Abschnitt „Fortsetzung in neuem Chat" der Vorversion vorgesehen. `const VER='6.27'` wurde vor der Änderung gegengeprüft (stimmte).

> **Hinweis zu dieser Revision:** Diese Aktualisierung basiert auf dem tatsächlich vorliegenden Code-Stand **v6.27** — jede hier beschriebene Änderung (v6.7–v6.27) wurde in diesem Chat direkt im Code umgesetzt und per `grep`/Funktionsansicht bzw. automatisiertem Playwright-Test am Ende nochmal gegen die reale Datei verifiziert, nicht aus dem Gedächtnis rekonstruiert. Vor der nächsten Änderung trotzdem wie gehabt: aktuelle Datei hochladen und `const VER=` gegenprüfen.
>
...
- PWA-fähig — `apple-mobile-web-app-capable`, installierbar. **Seit v6.30 mit echtem Offline-Betrieb:** Service Worker (`sw.js`) mit network-first-Strategie für `index.html`/`manifest.json`/Sprachpakete (online immer frisch, offline letzter Stand) und cache-first für die Icons. Cache-Name an `VER` gekoppelt — alter Cache wird bei jedem Versionssprung automatisch gelöscht. **Offene Frage:** Warum der ursprüngliche Service Worker (vor v6.5) entfernt wurde, ist nicht dokumentiert und dem Nutzer nicht mehr erinnerlich (Stand v6.30) — falls das Problem wieder auftaucht, war es vermutlich derselbe ungeklärte Grund.
...
- **Neu seit v6.27:** SRS-Verteilung im Zeitverlauf (eigener Unterabschnitt unten)

### SRS-Verteilung im Zeitverlauf (neu seit v6.27)

Neue Karte unterhalb der SRS-Stufen-Verteilung: ein **vertikaler Regler** (`<input type="range">` mit `-webkit-appearance:slider-vertical` + `writing-mode:vertical-lr` als Fallback für Nicht-WebKit-Browser), oben = "Jetzt", je weiter nach unten gezogen desto weiter in der Vergangenheit. Zeigt für den gewählten Zeitpunkt dieselbe Balkendarstellung wie die normale SRS-Stufen-Karte (`renderSrsBars()`, jetzt aus `renderSRS()` herausgelöst und wiederverwendet).

**Kernidee — keine neue Datenerfassung nötig:** Die Verteilung wird **rückwirkend aus den seit v6.26 gespeicherten Sitzungs-"moves" rekonstruiert**, nicht separat aufgezeichnet:

1. Start beim aktuellen Stand: `stageMap` = aktuelle Stufe jeder Vokabel (Schlüssel `native+'_'+foreign`).
2. Gespeicherte Sitzungen (`sv6_sessions`) werden **neueste zuerst** durchlaufen. Für jede Sitzung wird jede ihrer `moves` **rückgängig gemacht**: `stageMap[key] = move.from` (statt `move.to`).
3. Nach jeder rückgängig gemachten Sitzung wird ein Zeitpunkt (`{ts, label, dist}`) gespeichert — das ist der Zustand *unmittelbar vor* dieser Sitzung.
4. Ergebnis-Array wird umgedreht (älteste zuerst), Regler-Index bildet direkt auf ein Element ab.

`distAtTime(stageMap, atTime)` zählt für jeden Zeitpunkt nur Vokabeln, die zu diesem Zeitpunkt schon existierten (`created <= atTime`) — verhindert, dass später hinzugefügte Wörter fälschlich in vergangenen Verteilungen auftauchen.

**Bekannte Grenzen (Nutzer bewusst in Kauf genommen):**
- Reglerpunkte sind an **tatsächliche gespeicherte Sitzungen** gebunden, keine gleichmäßige Zeitachse (Tage ohne Sitzung erzeugen keinen eigenen Punkt) — bei 100 gespeicherten Sitzungen (`SESSIONS_MAX`) ist das der maximale Rückblick.
- Zeitraum ist **kürzer** als beim 365-Tage-Log, da an die Sitzungs-Historie gekoppelt (v6.26 eingeführt) statt an das ältere, länger laufende `sv6log`.
- Gelöschte Vokabeln können nicht rekonstruiert werden (kein Datenrest, sobald aus `S.vocab` entfernt).
- Ohne gespeicherte Sitzungen (`sv6_sessions` leer) wird der Regler ausgeblendet und ein Hinweistext gezeigt.

*Getestet (Playwright): 2 simulierte Sitzungen mit bekannten Stufensprüngen (Hund 1→2→3, Katze 0→1, Baum 4→5) — alle 3 rekonstruierten Zeitpunkte (ältester/mittlerer/jetzt) stimmten exakt mit den erwarteten Verteilungen überein. Leer-Zustand (keine Sitzungen) zeigt korrekt den Hinweistext statt Regler.*
...
> **Wichtiger Hinweis aus diesem Chat:** Änderungen, die hier im Chat an der Datei vorgenommen werden, wirken sich **nicht automatisch** auf die Live-Version unter `loberes.github.io/Test/` aus. Die aktualisierte `index.html` muss jedes Mal manuell ins GitHub-Repo hochgeladen werden.

---

## Offene Punkte (besprochen, nicht umgesetzt)

Erledigt seit letzter Revision: Typografie-Fix, Sitzungs-Zusammenfassung → v6.26; SRS-Zeitverlauf → v6.27 (deckt Idee 2 unten teilweise ab, siehe Anmerkung).

**Weiterhin offen, nur als Ausblick genannt, nicht konkret beauftragt:**
1. **"Gezielt üben" mit Pro-Wort-Daten verbessern:** z. B. Filter nach "kürzlich häufig falsch" (aus Sitzungs-/Log-Daten) statt nur dem bisherigen globalen `wrong≥correct`-Filter (`dsOften`). Weiterhin nicht umgesetzt.
2. **Statistik-Seite um Pro-Wort-Auswertungen erweitern** (z. B. "Problemwörter"-Liste, anklickbar → direkt in Gezielt üben starten): Der SRS-Zeitverlauf (v6.27) ist ein erster Schritt in diese Richtung, aber keine vollständige Problemwörter-Auswertung — die konkrete Idee aus dem Chat ist also nur teilweise abgedeckt.
3. Weitere in einem früheren Chat genannte Ideen zur erweiterten Statistiknutzung (Verwechslungs-Paare auswerten, Auswertung nach Pack/Kategorie) — beide würden zusätzliches Tracking benötigen, das aktuell nicht existiert. Rein als Ideen im Raum, nicht angefragt.

---

## Changelog

### v6.30 — PWA-Umbau: Service Worker, Offline, Update-Banner, Install-Button
- **Neue Datei `sw.js`:** muss ab jetzt zusätzlich zu `index.html` und `manifest.json` im Repo-Root liegen, sonst greift nichts von Folgendem. Cache-Name `slovnik-v6.30` ist im Code hart an die App-Version gekoppelt.
- **Caching-Strategie:** Network-first mit Cache-Fallback für `index.html`, `manifest.json` und alle Sprachpaket-JSONs (`lang_*.json`, auch die Leerzeichen-Variante, siehe `fetchRelative()`) — online immer frische Daten, offline der zuletzt geladene Stand. Cache-first nur für `icon-192.png`/`icon-512.png` (ändern sich praktisch nie). Externe Requests (MyMemory-Übersetzungs-API) läuft der SW nicht dazwischen.
- **Alter "Seek-and-destroy"-Block entfernt:** Der bisherige Code, der bei jedem Start jeden Service Worker aktiv deregistriert hat, ist raus — sonst hätte er die neue Registrierung sofort wieder gekillt.
- **Update-Banner:** Wenn im Hintergrund eine neue Version installiert wurde, erscheint ein kleiner Banner unten ("Neue Version verfügbar" + Button) — der Nutzer entscheidet selbst, wann neu geladen wird (Nutzerentscheidung, nicht automatisch).
- **Install-Button für Android/Chrome:** In den Einstellungen erscheint oben ein "App installieren"-Button, aber nur wenn der Browser das `beforeinstallprompt`-Event feuert (also nie auf iOS — dort bleibt "Zum Home-Bildschirm" der Weg, das ist normales Verhalten, kein Bug).
- **Icons pragmatisch erweitert, keine neuen Bilddateien:** `apple-touch-icon` und Favicon zeigen auf das bestehende `icon-192.png` (kein eigenes 180×180 o. ä. generiert, da keine Quelldatei vorlag). Echte Multi-Size-Icons (180/152/32/16 px) wären mit Pillow im Sandbox-Environment technisch möglich, sobald eine hochauflösende Quelldatei (`icon-512.png`) hochgeladen wird — bisher nicht gemacht.
- **Bewusst nicht gemacht:** Handgebaute `apple-touch-startup-image`-Splashscreens pro Gerätegröße — seit einigen iOS-Versionen generiert Safari daraus ohnehin automatisch einen Splash aus Manifest-Icon + `background_color`, zusätzliche Bilddateien dafür hätten nur Umfang ohne echten Mehrwert gebracht.
- **Offene Frage ungeklärt:** Warum der ursprüngliche Service Worker vor v6.5 entfernt wurde, konnte der Nutzer nicht mehr sagen. Falls nach diesem Umbau ein Bug auftaucht, der an einen früheren Zustand erinnert (z. B. Stale-Content-Probleme) — das könnte derselbe ungeklärte Grund sein, dann bitte hier nachtragen.
- **Neue Standing Instruction:** Ab sofort bei jeder Versionsänderung **zwei** Stellen synchron hochzählen: `const VER` in `index.html` **und** `CACHE_VER` in `sw.js`. Wird das vergessen, liefert der Service Worker weiter alten Code aus, auch nach einem frischen Deploy.
- **Verifikation:** Node-Syntax-Check für `index.html` und `sw.js` einzeln, JSON-Check für `manifest.json`, Diff gegen v6.29 zeigt nur die beabsichtigten Zeilen. **Kein Playwright-Testlauf, kein echter Geräte-/Browser-Test** — bei einem Service Worker (im Gegensatz zu reinem UI-Code) ist das Risiko für unbemerkte Fehler deutlich höher, deshalb hier besonders wichtig: vor dem nächsten Feature auf einem echten Gerät testen (online laden → Flugmodus → neu laden sollte trotzdem gehen; nach einem echten Deploy mit Versionssprung sollte der Update-Banner erscheinen).

### v6.29
- **Stufen-Überblick nach der Sitzung:** In der Sitzungs-Zusammenfassung (nach Lernrunde und in der Sitzungs-Historie) zeigt ein neuer Abschnitt „Stufen-Überblick" die Gesamtverteilung der in dieser Sitzung geübten Wörter über die 8 Stufen — vorher (`startStage`) vs. nachher (`endStage`) je Stufe, als gepaarte Balken (blass=vorher, kräftig=nachher) mit Delta+Pfeil obendrüber. Ergänzt die bereits bestehende Pro-Wort-Bewegungsliste, ersetzt sie nicht.
- **Kein neuer Speicherbedarf:** Berechnet aus den ohnehin pro Sitzung erfassten `wordStats` (`startStage`/`endStage` je geübtem Wort), genau wie der SRS-Zeitverlauf (v6.27) aus vorhandenen Daten rekonstruiert.
- **Abwärtskompatibel:** Ältere gespeicherte Sitzungen (vor v6.29) haben keine `stageBefore`/`stageAfter`-Felder — der Abschnitt wird für diese einfach ausgeblendet, kein Fehler, keine Migration nötig.
- Neue i18n-Schlüssel `sessStageOverview`/`sessStageLegend` für alle drei UI-Sprachen (DE/EN/SK) ergänzt.
- Design wurde vorab als Skizze mit dem Nutzer abgestimmt (3 Varianten zur Auswahl, dann kombiniert: Variante 2 als Basis + Delta/Pfeil aus Variante 3 obendrüber).
- **Verifikation:** Node-Syntax-Check (gesamter Script-Block), Diff gegen v6.28 zeigt nur die beabsichtigten Zeilen. Kein Playwright-Testlauf (weiterhin kein Browser-Ausführungstool in dieser Umgebung verfügbar).

### v6.28
- **`manifest.json` wieder mit `index.html` verknüpft:** `<link rel="manifest" href="manifest.json">` im `<head>` ergänzt (fehlte, seit wann ist unklar — kommt in keinem Changelog-Eintrag v6.5–v6.27 vor). Die vom Nutzer bereitgestellte `manifest.json` selbst war inhaltlich korrekt (Name, Icons 192/512 „any maskable", `display:standalone`, Theme-Farbe, Kategorien) — ohne das Link-Tag konnte sie aber kein Browser finden. **Setzt voraus, dass `icon-192.png`/`icon-512.png` im selben Verzeichnis wie `index.html` liegen** — vom Nutzer beim Hochladen zu prüfen, hier nicht verifizierbar. Ändert nichts am fehlenden Service Worker (weiterhin kein Offline-Betrieb, siehe Abschnitt „Architektur").
- **Dokumentationskorrektur:** Falsche Ursachen-Behauptung im Abschnitt „Deployment" entfernt (fälschlich als Ursache mehrfacher Verwirrung "Fix wirkt nicht" das Testen des alten Live-Stands genannt — laut Nutzer lag es an einem Missverständnis, nicht daran). Der Kernpunkt (Deployment ist manuell, kein Auto-Sync zu GitHub Pages) bleibt unverändert bestehen.
- **Verifikation dieser Version:** JSON-Syntax von `manifest.json` geprüft (gültig). HTML/JS-Änderung ist eine einzelne Zeile (`<link>`-Tag), keine Logikänderung. **Kein Playwright-Testlauf** in dieser Sitzung (kein Browser-Ausführungstool verfügbar) — abweichend vom sonst in diesem Dokument beschriebenen Standard, hier explizit vermerkt statt stillschweigend behauptet.

### v6.27 — letzte Version des vorherigen Chats
- **SRS-Verteilung im Zeitverlauf:** neue Karte auf der Statistik-Seite mit vertikalem Regler ("Jetzt" oben, Vergangenheit unten). Rekonstruiert vergangene SRS-Stufen-Verteilungen rückwirkend aus den seit v6.26 gespeicherten Sitzungs-"moves" (`buildSrsTimeline()`, `distAtTime()`) — **kein zusätzlicher Speicher nötig**. `renderSRS()` intern aufgeteilt in `renderSrsBars()` (Balken zeichnen, wiederverwendbar) und `currentSrsDist()`. Details und bekannte Grenzen (kürzerer Zeitraum als Log, keine gelöschten Vokabeln rekonstruierbar) siehe Abschnitt „Statistik & Tages-Log", Unterabschnitt „SRS-Verteilung im Zeitverlauf".
- Dies war die **letzte Änderung in diesem Chat** — siehe Hinweis am Dokumentende zur Fortsetzung in einem neuen Chat.
...
## Hinweis: Fortsetzung in neuem Chat

Dieser Chat endet nach v6.27 auf Wunsch des Nutzers. Für den nächsten Chat:

1. **Diese Doku-Datei** (`SLOVNIK_DOKU_6_27.md`) und die aktuelle **`index.html`** (v6.27) zusammen hochladen — die Doku ist der verlässliche Kontext, nicht Chatverlauf-Erinnerung.
2. **Als Erstes tun:** `index.html` öffnen, `const VER=` gegenprüfen (muss `'6.27'` sein — falls nicht, hat der Nutzer zwischenzeitlich selbst etwas geändert oder eine andere Version hochgeladen, dann Vorsicht mit Annahmen aus dieser Doku).
3. **Interner Build-Workflow (3-Teile-Aufbau, Abschnitt „Architektur"):** existiert nur innerhalb einer laufenden Sitzung, muss im neuen Chat bei Bedarf neu aus der hochgeladenen `index.html` hergestellt werden (an den `<style>`/`<script>`-Grenzen extrahieren, siehe `src/build.js`-Beschreibung im Architektur-Abschnitt). Nicht automatisch vorhanden.
4. **Standing Instructions, die weiterhin gelten sollten** (bisher in diesem Chat etabliert, nicht in der App selbst gespeichert, also hier explizit vermerkt):
   - Diese Doku nach jeder Versionsänderung automatisch mit aktualisieren, ohne dass extra danach gefragt werden muss (siehe Hinweis-Box ganz oben im Dokument).
   - Vor größeren Umbauten kurz Rückfrage/Bestätigung einholen (ursprüngliche Nutzer-Präferenz aus sehr frühem Chat-Verlauf, nicht mit dieser Doku-Instanz neu bestätigt — im neuen Chat ggf. nochmal explizit erfragen, ob das weiterhin gilt).
   - Nach jeder Änderung: Versionsnummer hochzählen, Syntax-Check (Node `new Function()`-Test), wo sinnvoll automatisierter Playwright-Test vor Auslieferung.
   - Änderungen an der Datei wirken sich nicht automatisch auf die Live-Version bei GitHub Pages aus — Nutzer muss jedes Mal manuell hochladen (siehe Abschnitt „Deployment").
5. **Abschnitt „Offene Punkte"** enthält alle im Chat besprochenen, aber nicht umgesetzten Ideen — Ausgangspunkt, falls der Nutzer im neuen Chat dort weitermachen möchte.

---

*Dokumentation zuletzt aktualisiert: August 2026, basierend auf Code-Stand **v6.30** (`index.html` + `sw.js`). v6.7–v6.27 gegen die reale Datei verifiziert (Vorgänger-Chat, inkl. Playwright); v6.28–v6.30 nur per Syntax-Check/Codesichtung (siehe Changelog-Hinweise), nicht per Playwright — bei v6.30 (Service Worker) besonders relevant, siehe Hinweis dort.*
